...
var NotificationAlert = function (_React$Component) {
    ...
    _createClass(NotificationAlert, [{
                ...
                if (sNotify[i].key !== nNumber + "") {
                    if (sNotify[i].props.className.indexOf("fadeOutUp") !== -1) {
                        ...
                        if (noAnimate === undefined) {
                            ...
                            dNotify = _react2.default.cloneElement(sNotify[i], { className: "alert-with-icon" + animation });
                        } else {
                            dNotify = _react2.default.cloneElement(sNotify[i], { className: "alert-with-icon" });
                        }
                    }
                    notify.push(dNotify);
                } else {
                    if (noAnimate === undefined) {
                        dNotify = _react2.default.cloneElement(sNotify[i], { className: "alert-with-icon animated fadeOutUp" });
                        notify.push(dNotify);
                    }
                }
            }
        ...
        value: function notificationAlert(options) {
            ...
            var notification = _react2.default.createElement(_reactstrap.Alert, { color: options.type, className: 'alert-with-icon animated fadeInDown', toggle: function toggle() {
                    return _this2.onDismiss(nNumber, options.place);
                }, key: nNumber }, options.icon !== undefined ? _react2.default.createElement('span', { 'data-notify': 'icon', className: options.icon }) : null, _react2.default.createElement('span', { 'data-notify': 'message' }, options.message));
            if (options.place.indexOf("b") !== -1) {
